...
operator<< (tm_ostream& out, modification mod) {
  switch (mod->k) {
  case MOD_ASSIGN:
    return out << "assign (" << root (mod) << ", " << mod->t << ")";
  case MOD_INSERT:
    return out << "insert (" << root (mod) << ", " << index (mod) << ", "
               << mod->t << ")";
  case MOD_REMOVE:
    return out << "remove (" << root (mod) << ", " << index (mod) << ", "
               << argument (mod) << ")";
  case MOD_SPLIT:
    return out << "split (" << root (mod) << ", " << index (mod) << ", "
               << argument (mod) << ")";
  case MOD_JOIN:
    return out << "join (" << root (mod) << ", " << index (mod) << ")";
  case MOD_ASSIGN_NODE:
    return out << "assign_node (" << root (mod) << ", " << mod->t << ")";
  case MOD_INSERT_NODE:
    return out << "insert_node (" << root (mod) << ", " << argument (mod)
               << ", " << mod->t << ")";
  case MOD_REMOVE_NODE:
    return out << "remove_node (" << root (mod) << ", " << index (mod) << ")";
  case MOD_SET_CURSOR:
    return out << "set_cursor (" << root (mod) << ", " << index (mod) << ", "
               << mod->t << ")";
  default:
...
root (modification mod) {
  ...
  case MOD_ASSIGN:
    return mod->p;
  case MOD_INSERT:
    return path_up (mod->p);
  case MOD_REMOVE:
    return path_up (path_up (mod->p));
  case MOD_SPLIT:
    return path_up (path_up (mod->p));
  case MOD_JOIN:
    return path_up (mod->p);
...
index (modification mod) {
  switch (mod->k) {
  case MOD_INSERT:
    return last_item (mod->p);
  case MOD_REMOVE:
    return last_item (path_up (mod->p));
  case MOD_SPLIT:
    return last_item (path_up (mod->p));
  case MOD_JOIN:
    return last_item (mod->p);
...
clean_assign (tree t, path p, tree u) {
  if (is_nil (p)) return copy (u);
    ...
    if (j >= n)
      TM_FAILED ("clean_remove(): Invalid path."); // return copy(u);  // FIXME?
...
clean_remove (tree t, path p, int nr) {
  if (is_nil (p->next) && is_atomic (t)) {
    ...
    if (j >= n)
      TM_FAILED (
          "clean_remove: Invalid path"); // return t;  // FIXME? check whether
                                         // this is the right return value.
    tree r (t, n);
    for (i= 0; i < j; i++)
      r[i]= t[i];
...
clean_join (tree t, path p) {
  if (is_nil (p->next)) {
    ...
    if (is_atomic (s1)) u= tree (s1->label * s2->label);
    else {
      int n1= N (s1), n2= N (s2);
      u= tree (s1, n1 + n2);
      for (i= 0; i < n1; i++)
