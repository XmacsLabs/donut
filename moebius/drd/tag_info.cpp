...
int
drd_encode (tree t) {
  if (encode_table->contains (t)) return encode_table[t];
...
parent_info::operator== (const parent_info& pi) {
  return (type == pi.type) && (arity_mode == pi.arity_mode) &&
         (arity_base == pi.arity_base) && (arity_extra == pi.arity_extra) &&
         (child_mode == pi.child_mode) && (border_mode == pi.border_mode) &&
         (block == pi.block) && (with_like == pi.with_like) &&
         (var_type == pi.var_type) && (freeze_arity == pi.freeze_arity) &&
         (freeze_border == pi.freeze_border) &&
         (freeze_block == pi.freeze_block) && (freeze_with == pi.freeze_with) &&
...
child_info::operator== (const child_info& ci) {
  return (type == ci.type) && (accessible == ci.accessible) &&
         (writability == ci.writability) && (block == ci.block) &&
         (env == ci.env) && (freeze_type == ci.freeze_type) &&
         (freeze_accessible == ci.freeze_accessible) &&
         (freeze_writability == ci.freeze_writability) &&
         (freeze_block == ci.freeze_block) && (freeze_env == ci.freeze_env);
}
...
tag_info::operator tree () {
  if (rep->extra == "")
    return tree (TUPLE, as_tree (rep->pi), as_tree (rep->ci));
...
tag_info_rep::get_attribute (string which) {
  ...
  int i, n= N (extra);
  for (i= 0; i + 1 < n; i+= 2)
    if (extra[i] == which) return extra[i + 1];
  return "";
}
...
tag_info_rep::get_index (int child, int n) {
  ...
  case CHILD_BIFORM:
    ...
    break;
  case CHILD_DETAILED:
    if (((int) pi.arity_mode) <= ARITY_OPTIONS) r= child;
