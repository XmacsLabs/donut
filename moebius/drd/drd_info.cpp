...
drd_info::drd_info (string name) : rep (tm_new<drd_info_rep> (name)) {}
...
drd_info_rep::set_locals (tree t) {
  if (!is_func (t, COLLECTION)) return false;
  int i, n= N (t);
  for (i= 0; i < n; i++)
...
drd_info_rep::insert_point (tree_label l, int i, int n) {
  ...
  case ARITY_REPEAT:
    return (i >= 0) && ((i < ((int) pi.arity_base)) ||
                        ((i - pi.arity_base) % pi.arity_extra) == 0);
  case ARITY_VAR_REPEAT:
    return (i >= 0) &&
           ((i > (n - ((int) pi.arity_base))) || (i % pi.arity_extra == 0));
  }
  return false; // NOT REACHED
}
...
drd_info_rep::is_child_enforcing (tree t) {
  return ((info[L (t)]->pi.border_mode & BORDER_INNER) != 0) && (N (t) != 0);
}

bool
drd_info_rep::is_parent_enforcing (tree t) {
  return ((info[L (t)]->pi.border_mode & BORDER_OUTER) != 0) && (N (t) != 0);
}
...
drd_info_rep::get_attribute (tree_label l, string which) {
  ...
  if ((which == "name") && (val == "")) return as_string (l);
  return val;
}
...
drd_info_rep::get_syntax (tree t, path p) {
  ...
  else if (L (t) < START_EXTENSIONS || is_atomic (t)) return UNINIT;
  else {
...
drd_info_rep::all_accessible (tree_label l) {
  int i, n= N (info[l]->ci);
  for (i= 0; i < n; i++)
    if (info[l]->ci[i].accessible != ACCESSIBLE_ALWAYS) return false;
  return n > 0;
}
...
drd_info_rep::none_accessible (tree_label l) {
  int i, n= N (info[l]->ci);
  for (i= 0; i < n; i++)
    if (info[l]->ci[i].accessible != ACCESSIBLE_NEVER) return false;
  return true;
}
...
drd_info_rep::is_accessible_path (tree t, path p) {
  ...
  return is_accessible_child (t, p->item) && (p->item < N (t)) &&
...
drd_env_merge (tree env, tree t) {
  int i, n= N (t);
  for (i= 0; (i + 1) < n; i+= 2)
    if (is_atomic (t[i])) env= drd_env_write (env, t[i]->label, t[i + 1]);
  return env;
}
...
drd_info_rep::arg_access (tree t, tree arg, tree env, int& type, bool& found) {
  ...
  if (is_atomic (t)) return tree (ATTR);
    ...
    return arg_access (compound (t[0]->label, A (t (1, N (t)))), arg, env, type,
                       found);
    ...
    for (i= 0; i < n; i++) {
      ...
      if (aenv != "") {
        if (ctype != TYPE_INVALID) {
          type = ctype;
          found= cfound;
        }
        if (is_accessible_child (t, i)) return aenv;
      }
      else if (cfound /*type == TYPE_UNKNOWN*/ && ctype != TYPE_INVALID &&
               ctype != TYPE_UNKNOWN) {
        type = ctype;
        found= cfound;
  ...
  for (int i= 1; i < N (env); i+= 2)
    if (is_func (env[i], ARG, 1)) {
      for (int j= 0; j + 1 < N (macro); j++)
        if (macro[j] == env[i][0]) env[i]= tree (ARG, as_tree (j));
    }
}
...
drd_info_rep::heuristic_with_like (tree t, tree arg) {
  ...
  else if (t == tree (ARG, arg)) return true;
  else if (is_with_like (t)) return heuristic_with_like (t[N (t) - 1], arg);
  else return false;
}
...
drd_info_rep::heuristic_init_macro (string var, tree macro) {
  ...
  else if (n == 0 && is_atomic (macro[0]) && is_length (macro[0]->label)) {
...
minimal_arity (tree t, tree var) {
  ...
  else if (is_func (t, ARG, 2) && (t[0] == var)) return as_int (t[1]) + 1;
...
drd_info_rep::heuristic_init (hashmap<string, tree> env2) {
  ...
  while (flag) {
    ...
    while (it->busy ()) {
      ...
      else flag= heuristic_init_parameter (var, val) | flag;
    }
    if ((round++) == 10) {
